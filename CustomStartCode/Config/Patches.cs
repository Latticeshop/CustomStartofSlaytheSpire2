    ...
    public static void Install(HarmonyLib.Harmony harmony)
    {
        ...
        try
        {
            ...
            if (loadRunMethod != null)
            {
                ...
                Logger.Info("[CustomStart] 继续游戏遗物恢复补丁安装成功 (NGame.LoadRun)");
            }
            else
            {
                Logger.Warn("[CustomStart] 找不到 NGame.LoadRun 方法");
            }
        }
        catch (Exception ex)
        {
            Logger.Error($"[CustomStart] 继续游戏遗物恢复补丁安装失败: {ex.Message}");
        }
        ...
        public static void ApplyConfigToPlayer(Player __instance, RunState? runState = null)
        {
            try
            {
                ...
                if (config.StartingGold > 0 && __instance.Gold != config.StartingGold)
                {
                    ...
                    Logger.Info($"[CustomStart] 已应用初始金币: {config.StartingGold}（角色 {characterId}）");
                }
                if (config.MaxHp > 0)
                {
                    try
                    {
                        ...
                        Logger.Info($"[CustomStart] 已应用血量上限: {config.MaxHp}（角色 {characterId}）");
                    }
                    catch (Exception ex)
                    {
                        Logger.Warn($"[CustomStart] 应用血量上限失败: {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
